...
void print_input_file(char*); // Maybe delete this function later
...
int main() {
	...
	// Free memory
	free_strings_arr(data_strings_arr, num_vars);	
	free_ints_arr(data_ints_arr, num_vars);
	return 0;
...
int** strings_arr_to_ints_arr(char** data_strings_arr, int num_vars, int num_nodes) {
	int** ints_arr = calloc(num_vars, sizeof(int*));
	for(int i = 0; i < num_vars; i++) {
